          ...
          >
            ...
            <details className="group rounded-xl border border-slate-200 bg-slate-50 p-4">
              <summary className="cursor-pointer list-none text-sm font-bold text-slate-800 marker:hidden">
                <span className="flex items-center justify-between">Refine results <span className="text-brand-700 transition group-open:rotate-45">+</span></span>
              </summary>
              <div className="mt-4 space-y-4 border-t border-slate-200 pt-4">
                <Field label="Preferred state" htmlFor="state">
                  <select id="state" value={state} onChange={(event) => setState(event.target.value)} className="field-control">
                    <option value="">All India</option>
                    {INDIAN_STATES.map((item) => <option key={item}>{item}</option>)}
                  </select>
                </Field>
                <Field label="Course or branch" htmlFor="course">
                  <input id="course" value={course} onChange={(event) => setCourse(event.target.value)} className="field-control" placeholder="e.g. Computer Science" maxLength={100} />
                </Field>
                <Field label="Quota" htmlFor="quota">
                  <select id="quota" value={quota} onChange={(event) => setQuota(event.target.value)} className="field-control">
                    <option value="">Any quota</option>
                    {QUOTAS.map((item) => <option key={item}>{item}</option>)}
                  </select>
                </Field>
                <Field label="Cutoff year" htmlFor="year">
                  <input id="year" type="number" min="2000" max={new Date().getFullYear() + 1} value={year} onChange={(event) => setYear(event.target.value)} className="field-control" placeholder="Newest available" />
                </Field>
              </div>
            </details>
